...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    foreach ($has_rule as $field){
        if (isset($_POST[$field]) || !isset($_POST[$field])) {
            $validation = validateNumeric($field);
            if (!$validation) {
                $errors_rules[] = $field;
            }
    ...
    if (!count($errors_rules) && !count($errors_required)) {

    }
} ?>
<pre>
    <?php var_dump($_POST, $errors_rules);
    ?>
</pre>
...
<main>
    ...
    <form class="form form--add-lot container <?php echo !empty($errors_required) || !empty($errors_rules) ? 'form--invalid' : '' ?>"
          action="/add.php" method="post">
        <h2>Добавление лота</h2>
        <div class="form__container-two">
            <div class="form__item <?php echo in_array('lot-name', $errors_required) ? 'form__item--invalid' : ''; ?>">
            ...
            <div class="form__item <?php echo in_array('category', $errors_required) ? 'form__item--invalid' : ''; ?>">
                <label for="category">Категория</label>
                <select id="category" name="category">
                    <option  selected value="">Выберите категорию</option>
                    <?php foreach ($cats as $cat): ?>
                        <option value="<?= $cat; ?>" <?php echo $_POST['category'] == $cat ? 'selected' : '' ?>><?= $cat; ?></option>
                    <?php endforeach; ?>
                </select>
        ...
        <div class="form__item form__item--wide <?php echo in_array('message', $errors_required) ? 'form__item--invalid' : ''; ?>">
        ...
        <div class="form__container-three">
            <div class="form__item form__item--small <?php echo in_array('lot-rate', $errors_required) ? 'form__item--invalid' : ''; ?>">
            ...
            <div class="form__item form__item--small <?php echo in_array('lot-step', $errors_required) ? 'form__item--invalid' : ''; ?>">
            ...
            <div class="form__item <?php echo in_array('lot-date', $errors_required) ? 'form__item--invalid' : ''; ?>">
                <label for="lot-date">Дата завершения</label>
                <input class="form__input-date" id="lot-date" type="text" name="lot-date" placeholder="20.05.2017" value="<?= $_POST['lot-date'] ?? ''; ?>">
